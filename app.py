...
if not st.session_state['logged_in']: 
    ...
    with tabs[1]: 
        ...
        if 'df' in st.session_state and not st.session_state['df'].empty: 
            ...
            n_q, n_s, n_e, res = 0, "", "", st.text_input("說明原因") 
            if act == "更正數量": n_q = st.number_input("新數量", step=1)
            elif act == "貨況轉換": n_q = st.number_input("轉換數量", step=1); n_s = st.radio("新貨況", ["良品", "不良品"])
            elif act == "效期更正": n_e = st.text_input("新有效期限")
            ...
            if st.button("⚠️ 送出更正申請"): 
                ...
                for _, row in selected.iterrows(): 
                    conn.execute("INSERT INTO change_requests (item_id, action, old_qty, new_qty, new_status, new_expiry, reason, status) VALUES (?, ?, ?, ?, ?, ?, ?, '審核中')", 
                                 (int(row['ID']), act, int(row['數量']), int(n_q), n_s, n_e, f"{act}: {res}")) 
