...
if not st.session_state['logged_in']:
    ...
    with tab1:
        ...
        if st.button("進入系統"):
            ...
            conn.close()
    with tab2:
        reg_name = st.text_input("真實姓名").strip(); reg_pwd = st.text_input("自訂密碼", type="password")
        if st.button("註冊"):
            conn = get_db_connection()
            try: conn.execute('INSERT INTO users VALUES (?, ?, ?, ?)', (reg_name, reg_pwd, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "一般用戶")); conn.commit(); st.success("成功")
    ...
    with tabs[0]:
        ...
        if not st.session_state.get('current_batch'):
            ...
            if st.button("開始作業"):
                ...
                st.session_state.update({'current_batch': bid, 'current_chan': chan}); st.rerun()
        else:
            ...
            if st.button("儲存"):
                conn = get_db_connection()
                conn.execute('INSERT INTO return_items (batch_id, barcode, return_type, expiry_date, quantity, quality_status, operator, approval_status, created_at, remark) VALUES (?, ?, ?, ?, ?, ?, ?, "已確認", ?, ?)', 
                             (st.session_state['current_batch'], barcode, r_type, exp, qty, qual, st.session_state['username'], datetime.now().strftime("%Y-%m-%d %H:%M:%S"), remark))
                conn.commit(); conn.close(); st.toast("✅")
            
            c1, c2 = st.columns(2)
            if c1.button("返回 / 暫停作業", key="back-btn"): st.session_state.pop('current_batch'); st.rerun()
            if c2.button("結束作業並關單", key="close-btn"): 
                conn = get_db_connection(); conn.execute("UPDATE return_batches SET status = '已完成' WHERE batch_id = ?", (st.session_state['current_batch'],)); conn.commit(); conn.close()
                st.session_state.pop('current_batch'); st.rerun()

    with tabs[1]:
        s_batch = st.text_input("查詢批號")
        if st.button("查詢"):
            conn = get_db_connection()
            df = pd.read_sql_query("SELECT id, created_at, channel, batch_id, barcode, return_type, expiry_date, quantity, quality_status, damage_reason, operator, approval_status, remark FROM return_items i LEFT JOIN return_batches b ON i.batch_id = b.batch_id WHERE i.batch_id LIKE ?", conn, params=(f"%{s_batch}%",))
            df.columns = ["ID", "日期", "通路", "退貨單號", "商品條碼", "箱散出", "效期", "數量", "良品不良品", "異常原因", "作業員", "訂單狀態", "時間"]
            conn.close(); st.session_state['df'] = df
        if 'df' in st.session_state:
            st.dataframe(st.session_state['df'], use_container_width=True) # 唯讀表格
