    ...
    with tabs[0]:
        if not st.session_state.get('current_channel'):
            ...
            if st.button("鎖定並開始作業", use_container_width=True):
                if chan != "請選擇...":
                    bid = "Back" + datetime.now().strftime("%Y%m%d%H%M")
                    conn = get_db_connection(); conn.execute("INSERT INTO return_batches VALUES (?, ?, ?, '作業中')", (bid, chan, datetime.now().strftime("%Y%m%d")))
                    conn.commit(); conn.close(); st.session_state.update({'current_batch_id': bid, 'current_channel': chan}); st.rerun()
        else:
    ...
    with tabs[1]:
        ...
        if 'df' in st.session_state and not st.session_state['df'].empty:
            ...
            if not selected.empty:
                st.subheader("🛠️ 異常修正區")
                act = st.selectbox("動作", ["更正數量", "貨況轉換", "刪除資料"])
                n_q, n_s, res = 0, "", ""
                if act == "更正數量": n_q = st.number_input("新數量", step=1); res = st.text_input("說明原因")
                elif act == "貨況轉換": n_q = st.number_input("轉換數量", step=1); n_s = st.radio("新貨況", ["良品", "不良品"]); res = ", ".join(st.multiselect("不良原因", DAMAGE_REASONS)) if n_s == "不良品" else ""
                if st.button("⚠️ 送出申請"):
                    conn = get_db_connection()
                    ...
                    conn.commit(); conn.close(); show_alert("⚠️ 已申請成功，待主管簽核。")
        ...
        if st.button("🟢 批量同意"):
            ...
            for _, row in reviewed_df.iterrows():
                if row.get("同意"):
                    ...
                    if row['action'] == "更正數量": conn.execute("UPDATE return_items SET quantity = ? WHERE id = ?", (row['new_qty'], row['item_id']))
