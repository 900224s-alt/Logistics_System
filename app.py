...
<style>
    ...
    [data-testid="stDataEditor"] *, [data-testid="stDataFrame"] * { font-size: 16px !important; font-weight: 500 !important; }
    .glide-data-grid { font-family: inherit !important; }
    div[data-testid="stSelectbox"] *, div[data-testid="stNumberInput"] * { font-size: 16px !important; }
</style>
...
def init_connection_pool():
    pool = SimpleConnectionPool(
        ...
        user="postgres.zlmoazvoxadasdbdwbsl", password="ALS56606120",
...
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS users (
            username TEXT PRIMARY KEY, password TEXT, register_date TEXT, role TEXT, status TEXT DEFAULT 'pending'
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS return_batches (
            batch_id TEXT PRIMARY KEY, channel TEXT, register_date TEXT, status TEXT
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS return_items (
            id SERIAL PRIMARY KEY, batch_id TEXT, barcode TEXT, return_type TEXT, expiry_date TEXT, 
            quantity INTEGER, quality_status TEXT, damage_reason TEXT, operator TEXT, approval_status TEXT, created_at TEXT, remark TEXT
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS change_requests (
            req_id SERIAL PRIMARY KEY, item_id INTEGER, action TEXT, old_qty INTEGER, new_qty INTEGER, 
...
if 'logged_in' not in st.session_state:
    if "user" in st.query_params and "role" in st.query_params:
        st.session_state.update({'logged_in': True, 'username': st.query_params["user"], 'is_admin': st.query_params["role"] == "admin"})
    else:
        st.session_state['logged_in'] = False

st.title("📦 物流退貨點收系統 (極速雲端版)")
...
if not st.session_state['logged_in']:
    ...
    with tab1:
        ...
        if st.button("進入系統"):
            ...
            if user and user['password'] == login_pwd:
    ...
    with tab2:
        ...
        if st.button("建立帳號"):
            ...
            try:
                ...
                conn.commit(); st.success("註冊成功！請等待審核。")
    ...
    with col_refresh:
        if st.button("🔄 刷新數據", help="強制清除快取，即時抓取雲端最新數據"):
            clear_query_cache()
            st.toast("雲端數據已同步至最新狀態！")
            st.rerun()
            
    st.sidebar.write(f"👤 作業員：**{st.session_state['username']}** | 👑 職位：**{'管理者' if st.session_state.get('is_admin') else '一般用戶'}**")
    ...
    if st.session_state.get('is_admin'):
        tab_labels.extend([f"🔔 主管審核工作台 ({pending_req_count})" if pending_req_count > 0 else "🔔 主管審核工作台", 
                           f"👥 員工權限維護 ({pending_user_count})" if pending_user_count > 0 else "👥 員工權限維護"])
    ...
    with tabs[0]:
        ...
        if not st.session_state.get('current_batch_id'):
            ...
            if st.button("鎖定並開始作業"):
                ...
                cursor.close(); release_db_connection(conn)
                st.session_state.update({'current_batch_id': bid, 'current_channel': chan, 'batch_created_in_db': False, 'today_date_str': today})
                st.rerun()
        else:
            ...
            if st.button("💾 儲存並繼續新增", use_container_width=True, type="primary"):
                if b_input in BARCODE_ALERTS:
                    for alert in BARCODE_ALERTS[b_input]: st.warning(f"⚠️ 條碼 {b_input} 提醒：{alert}")
                ...
                if not st.session_state.get('batch_created_in_db'):
                    ...
                    if cursor.fetchone()[0] == 0:
                        cursor.execute("INSERT INTO return_batches VALUES (%s, %s, %s, '作業中')", (st.session_state['current_batch_id'], st.session_state['current_channel'], st.session_state['today_date_str']))
            ...
            if c1.button("🔙 返回 / 暫停作業", use_container_width=True, key="back-btn"):
                st.session_state.update({'current_channel': "", 'current_batch_id': "", 'last_item_id': None, 'last_count': None}); st.rerun()
        ...
        with st.expander("⚙️ 篩選條件設定", expanded=True):
            ...
            if st.button("執行查詢", type="primary"):
                ...
                if not df.empty:
                    ...
                    st.session_state['df'] = pd.DataFrame(); st.warning("查無符合條件的資料")
        
        if 'df' in st.session_state and not st.session_state['df'].empty:
            disabled_cols = [c for c in st.session_state['df'].columns if c != "選取"]
            ...
            with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                edited_df.to_excel(writer, index=False, sheet_name='點收紀錄')
            excel_data = buffer.getvalue()
            
            st.download_button(
                label="📥 下載目前查詢表格為 Excel",
                data=excel_data,
                file_name=f"退貨點收紀錄_{get_tw_now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                key="dl_history_excel"
            )
            ...
            if st.button("⚠️ 送出異常修正申請", use_container_width=True):
                if selected.empty:
                    st.error("❌ 請先在上方表格勾選欲更正的資料行！")
                else:
                    conn = get_db_connection(); cursor = conn.cursor()
                    for _, row in selected.iterrows():
                        cursor.execute("""
                            INSERT INTO change_requests (item_id, action, old_qty, new_qty, new_status, new_expiry, reason, status, applicant) 
                            VALUES (%s, %s, %s, %s, %s, %s, %s, '審核中', %s)
                        """, (int(row['ID']), act, int(row['數量']), int(n_q), n_status, n_expiry, n_reason, st.session_state['username']))
                    conn.commit(); cursor.close(); release_db_connection(conn)
                    st.success("✅ 申請已成功送出，請靜待主管審核工作台確認！")
                    clear_query_cache()
        ...
        with tabs[2]:
            ...
            with review_container:
                ...
                if not review_df.empty:
                    ...
                    display.columns = ['單號', '商品條碼', '更正動作', '原數量', '申請新數量', '變更貨況', '變更效期', '變更原因', '申請人']
                    display.insert(0, "同意", False)
                    
                    reviewed = st.data_editor(display, disabled=[c for c in display.columns if c != "同意"], hide_index=True, key="admin_review_editor")
                    
                    buffer_rev = io.BytesIO()
                    with pd.ExcelWriter(buffer_rev, engine='xlsxwriter') as writer:
                        reviewed.to_excel(writer, index=False, sheet_name='待審核清單')
                    review_excel = buffer_rev.getvalue()
                    
                    st.download_button(
                        label="📥 下載待審核清單為 Excel",
                        data=review_excel,
                        file_name=f"待主管審核清單_{get_tw_now().strftime('%Y%m%d')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="dl_review_excel"
                    )
                    
                    if st.button("🟢 執行批量審核通過", type="primary", use_container_width=True):
                        conn = get_db_connection(); cursor = conn.cursor()
                        processed_count = 0
                        ...
                        if processed_count > 0:
                            st.success(f"✅ 審核完成，已處理 {processed_count} 筆更正申請！")
                            clear_query_cache(); st.rerun()
                else:
