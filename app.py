...
def init_db():
    ...
    # 確保 return_items 存在且包含 expiry_date
    ...
    try:
        cursor.execute("ALTER TABLE return_items ADD COLUMN expiry_date TEXT")
    except:
        pass
    ...
    with tabs[0]:
        if not st.session_state.get('current_channel'):
            ...
            if st.button("💾 儲存並繼續新增", use_container_width=True, type="primary"):
                final_exp = "無效期" if r_type == "箱出" else exp_date
                if r_type != "箱出" and (not final_exp or (final_exp != "無效期" and (len(final_exp) != 8 or not final_exp.isdigit()))):
                    st.error("❌ 效期格式錯誤，應為 8 位數字 (例如 20260618) 或 '無效期'")
                else:
                    conn = get_db_connection()
                    conn.execute('''INSERT INTO return_items (batch_id, barcode, return_type, expiry_date, quantity, quality_status, damage_reason, operator, approval_status, created_at, remark) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                                 (st.session_state['current_batch_id'], b_input, r_type, final_exp, qty, qual, reason, st.session_state['username'], '已確認', datetime.now().strftime("%Y-%m-%d %H:%M:%S"), remark))
                    conn.commit(); conn.close()
                    st.success("✅ 儲存成功！")
