...
if not st.session_state['logged_in']: 
    ...
    with tabs[0]: 
        if not st.session_state.get('current_channel'): 
            ...
            if c2.button("🛑 結束作業並關單", use_container_width=True, key="close-btn"): conn = get_db_connection(); conn.execute("UPDATE return_batches SET status = '已完成' WHERE batch_id = ?", (st.session_state['current_batch_id'],)); conn.commit(); conn.close(); st.session_state.update({'current_channel': "", 'current_batch_id': ""}); st.rerun() 
