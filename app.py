...
# ==========================================
    ...
    with tabs[0]:
        if st.session_state['current_channel'] == "":
            ...
            # --- 條碼互動區 (HTML + Streamlit 橋接) ---
            components.html(
                """
                <div id="scanner_container" style="position: relative; width: 100%; font-family: sans-serif;">
                    <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 10px;">
                        <button id="scan_btn" style="flex: 1; padding: 12px; font-size: 16px; background-color: #ff4b4b; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">
                            📷 啟動相機掃描
                        </button>
                    </div>
                    <div id="interactive" style="display: none; width: 100%; height: 250px; border: 3px solid #ff4b4b; border-radius: 8px; background: #000;"></div>
                    <button id="close_btn" style="display: none; margin-top: 10px; width: 100%; padding: 10px; background-color: #555; color: white; border: none; border-radius: 4px;">❌ 關閉相機</button>
                </div>
                ...
                <script>
                    ...
                    scanBtn.onclick = () => {
                        ...
                        Quagga.init({inputStream: {type: "LiveStream", target: interactive}, decoder: {readers: ["code_128_reader", "ean_reader"]}}, (err) => { Quagga.start(); });
                    };
            ...
            # 💡 關鍵修改：將條碼輸入框直接綁定到 session_state['barcode_field']
            # 當 HTML 掃碼成功會寫入 session_state，手動輸入也會直接覆蓋更新該值
            st.session_state['barcode_field'] = st.text_input(
                "最終確認條碼（相機掃入或手動輸入）", 
                value=st.session_state['barcode_field']
            )
            ...
            with col1:
                if st.button("💾 儲存並繼續新增", use_container_width=True, type="primary"):
                    if not st.session_state['barcode_field']: st.error("❌ 請先刷取或輸入條碼！")
                        ...
                        st.session_state['barcode_field'] = "" # 清空條碼欄位供下一筆使用
