...
if not st.session_state['logged_in']:
    ...
    with tabs[1]:
        ...
        if 'df' in st.session_state and not st.session_state['df'].empty:
            ...
            if act == "更正數量": new_q = st.number_input("輸入新數量"); res = st.text_input("說明原因")
            elif act == "貨況轉換": new_q = st.number_input("轉換數量"); new_status = st.radio("轉換為", ["良品", "不良品"])
    ...
    with tabs[2]:
        ...
        app_id = st.number_input("輸入欲處理 ID", min_value=1, step=1)
        if st.button("🟢 同意變更"):
