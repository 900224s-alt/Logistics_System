...
if not st.session_state['logged_in']:
    ...
    with tabs[1]:
        ...
        if 'df' in st.session_state and not st.session_state['df'].empty:
            ...
            n_q = st.number_input("新數量", step=1); n_s = st.text_input("新貨況/效期"); res = st.text_input("說明原因")
            if st.button("⚠️ 送出更正申請"):
                conn = get_db_connection()
                for _, row in selected.iterrows():
                    conn.execute("INSERT INTO change_requests (item_id, action, old_qty, new_qty, new_status, new_expiry, reason, status) VALUES (?, ?, ?, ?, ?, ?, ?, '審核中')", (int(row['ID']), act, int(row['數量']), int(n_q), n_s if act == "貨況轉換" else "", n_s if act == "效期更正" else "", res))
