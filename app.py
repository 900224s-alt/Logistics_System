...
if not st.session_state['logged_in']: 
    ...
    for _, row in selected.iterrows(): 
        # 確保此處 VALUES 有 8 個佔位符 (?, ?, ?, ?, ?, ?, ?, ?)
        # 並確保傳入的 tuple 包含 8 個變數
        conn.execute("""INSERT INTO change_requests 
    ...
    with tabs[2]: 
        st.header("🔔 主管審核工作台") 
        conn = get_db_connection()
        query = """SELECT c.*, i.batch_id, i.barcode, i.created_at as apply_time, i.operator as applicant 
            ...
            if st.button("🟢 批量處理同意"): 
                ...
                for i, row in reviewed_df.iterrows(): 
                    if row.get("同意"): 
                        ...
                        item = conn.execute("SELECT * FROM return_items WHERE id = ?", (item_id,)).fetchone() 
                        if item: 
                            if action == "刪除資料": conn.execute("DELETE FROM return_items WHERE id = ?", (item_id,)) 
                            elif action == "更正數量": conn.execute("UPDATE return_items SET quantity = ? WHERE id = ?", (int(row['修正後數量']), item_id)) 
                            elif action == "效期更正": conn.execute("UPDATE return_items SET expiry_date = ?, quantity = ? WHERE id = ?", (review_df.iloc[i]['new_expiry'], int(row['修正後數量']), item_id))
                            elif action == "貨況轉換": 
                                old_q, new_q = int(item['quantity']), int(row['修正後數量'])
                                conn.execute("UPDATE return_items SET quantity = ? WHERE id = ?", (old_q - new_q, item_id))
                            ...
                            conn.execute("UPDATE change_requests SET status = '已確認' WHERE req_id = ?", (req_id,)) 
